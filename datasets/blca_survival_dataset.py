...
class BlcaSurvivalDataset(Dataset):
    def __init__(self, feats_path: str, labels_csv: str, split_file: str, split: str, n_bins: int = 4):
        ...
        # Create discrete time bins using quantiles of uncensored events (same as CRC)
        ...
        # Assign bin indices (1-based, same as CRC)
        ...
        if num_missing > 0:
            logger.warning(f"BLCA: skipping {num_missing} slides with missing feature files under {self.feats_path}")
        self.labels_df = self.labels_df[exists_mask].reset_index(drop=True)

        # Store metadata (same as CRC)
        ...
        logger.info(f"BLCA {split} dataset: {len(self.slide_ids)} samples")
    ...
    def __getitem__(self, idx):
        ...
        # Load features from HDF5 file (same as CRC)
        feat_file = os.path.join(self.feats_path, f"{slide_id}.h5")
        ...
        # Convert to torch tensors (same as CRC)
        features = torch.from_numpy(features).float()
        
        # Create survival dict (same as CRC)
        survival_dict = {
    ...
    def collate_fn(self, batch):
        """
        Custom collate function for batching (align with CRC):
        ...
        # Keep the same output structure as CRC
        return batch_features, batch_survival

def create_blca_survival_dataloaders(
    feats_path: str,
    splits_dir: str,
    split_idx: int,
    batch_size: int = 16,
    ...
    """
    Create BLCA survival data loaders for train/val/test
    ...
    """
    labels_csv = os.path.join(splits_dir, '..', 'tcga_blca_all_clean.csv')
    split_file = os.path.join(splits_dir, f'splits_{split_idx}.csv')
    
    # Create datasets
    train_ds = BlcaSurvivalDataset(feats_path, labels_csv, split_file, 'train', n_bins)
    val_ds = BlcaSurvivalDataset(feats_path, labels_csv, split_file, 'val', n_bins)
    
    # For BLCA, use val as test since there's no separate test split
    test_ds = val_ds
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Test BLCA survival dataset')
    ...
    try:
        ...
        print('✅ BLCA dataset loading successful!')
