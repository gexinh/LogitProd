...
class LuscSurvivalDataset(Dataset):
    def __init__(self, feats_path: str, labels_csv: str, split_file: str, split: str, n_bins: int = 4):
        """
        LUSC survival dataset
        
        Args:
            feats_path: Path to features directory
            labels_csv: Path to labels CSV file
            split_file: Path to split CSV file
            split: 'train', 'val', or 'test'
            n_bins: Number of time bins for discrete survival analysis
        """
        ...
        def _feat_exists(slide_id: str) -> bool:
            # For LUSC, features are in patches subdirectory
            feat_file = os.path.join(self.feats_path, 'patches', f"{slide_id}_patches.h5")
            return os.path.exists(feat_file)

        exists_mask = self.labels_df['slide_id'].apply(_feat_exists)
        num_missing = int((~exists_mask).sum())
        if num_missing > 0:
            logger.warning(f"LUSC: skipping {num_missing} slides with missing feature files under {self.feats_path}")
        ...
        logger.info(f"LUSC {split} dataset: {len(self.slide_ids)} samples")
    ...
    def __getitem__(self, idx):
        ...
        # Load features from HDF5 file (LUSC has patches in subdirectory)
        feat_file = os.path.join(self.feats_path, 'patches', f"{slide_id}_patches.h5")
        if not os.path.exists(feat_file):
            raise FileNotFoundError(f"Feature file not found: {feat_file}")
        
        with h5py.File(feat_file, 'r') as f:
            # LUSC patches file structure might be different, try common keys
            if 'features' in f.keys():
                features = f['features'][:]  # Shape: (N_patches, feature_dim)
            elif 'patches' in f.keys():
                features = f['patches'][:]  # Alternative key name
            else:
                # If no standard key, use the first dataset
                key = list(f.keys())[0]
                features = f[key][:]
...
def create_lusc_survival_dataloaders(
    feats_path: str,
    splits_dir: str,
    split_idx: int,
    batch_size: int = 16,
    ...
    """
    Create LUSC survival data loaders for train/val/test
    
    Args:
        feats_path: Path to features directory
        splits_dir: Directory containing split files (e.g., /home/laq/scratch/LUSC_split)
        split_idx: Split index (0-4 for 5-fold CV)
    ...
    """
    labels_csv = os.path.join(splits_dir, 'tcga_lusc_all_clean.csv')
    split_file = os.path.join(splits_dir, 'tcga_lusc', f'splits_{split_idx}.csv')
    
    # Create datasets
    train_ds = LuscSurvivalDataset(feats_path, labels_file, split_file, 'train', n_bins)
    val_ds = LuscSurvivalDataset(feats_path, labels_file, split_file, 'val', n_bins)
    
    # For LUSC, use test split if available, otherwise use val
    if os.path.exists(split_file):
        test_ds = LuscSurvivalDataset(feats_path, labels_file, split_file, 'test', n_bins)
    else:
        test_ds = val_ds
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Test LUSC survival dataset')
    ...
    try:
        ...
        print('✅ LUSC dataset loading successful!')
