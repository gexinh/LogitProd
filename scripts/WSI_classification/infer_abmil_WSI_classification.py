#!/usr/bin/env python3
"""
ABMIL Model Inference - GitHub Release Version
...
Usage example:
  python infer_abmil.py --dataset crc \
...
# -------------------------------
def main():
    parser = argparse.ArgumentParser(description="Re-infer ABMIL best models and save results")
    parser.add_argument('--dataset', type=str, required=True, choices=['crc', 'bracs', 'bracs_7class', 'brca', 'panda'],
                        help='Dataset type')
    ...
    if DATASET_TYPE == 'bracs':
        ...
        n_classes = 6  # PANDA: 6-class classification
    else:
        n_classes = 2  # Default: binary classification

    # PANDA currently uses only one splits_0_k.csv, so run 1 split; other datasets use args.num_splits
    num_splits = 1 if DATASET_TYPE == 'panda' else args.num_splits
    ...
    for MODEL_NAME, model_params in MODEL_HYPERPARAMS.items():
        ...
        print(f"\n===== Re-infer model {MODEL_NAME} on {DATASET_TYPE} ({num_splits} folds) =====")
        all_results = []
        for split_idx in range(num_splits):
            ...
            # Build loaders for this split (match training splits)
            if DATASET_TYPE == 'crc':
                if not CRC_AVAILABLE:
                    print("crc_dataset not available"); continue
                train_loader, val_loader, test_loader = create_crc_dataloaders(
                    feats_path=feats_path, splits_dir=splits_dir, split_idx=split_idx, batch_size=args.batch_size)
            elif DATASET_TYPE == 'bracs':
                if not BRACS_AVAILABLE:
                    print("bracs_dataset not available"); continue
                train_loader, val_loader, test_loader = create_bracs_dataloaders(
                    ...
                    batch_size=args.batch_size, binary_classification=False)
            elif DATASET_TYPE == 'brca':
                if not BRCA_AVAILABLE:
                    print("brca_dataset not available"); continue
                train_loader, val_loader, test_loader = create_brca_dataloaders(
                    feats_path=feats_path, splits_dir=splits_dir, split_idx=split_idx, batch_size=args.batch_size)
            elif DATASET_TYPE == 'panda':
                if not PANDA_AVAILABLE:
                    print("panda_dataset not available"); continue
                # PANDA only depends on splits_0_k.csv, so split_idx is not used
                train_loader, val_loader, test_loader = create_panda_dataloaders(
                    feats_path=feats_path, splits_dir=splits_dir, batch_size=args.batch_size)
            else:
                if not BRACS_7CLASS_AVAILABLE:
                    print("bracs_7class_dataset not available"); continue
