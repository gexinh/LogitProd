#!/usr/bin/env python3
"""
ABMIL Training for Gene Mutation Classification - GitHub Release Version

This script trains an Attention-Based Multiple Instance Learning (ABMIL) model
on whole slide images (WSI) for gene mutation binary classification tasks.
It runs cross-validation splits and saves slide-level features for downstream fusion tasks.

Key characteristics:
  - Trains ABMIL model on patch-level features extracted from pre-trained models.
  - Performs 5-fold cross-validation for gene mutation binary classification.
  ...
  - Uses softmax probabilities for AUC calculation (binary classification).
...
Usage example:
  python train_abmil_genemutation.py \
      --target_gene TP53 \
      --feats_path /path/to/features_{MODEL_NAME} \
      --splits_dir /path/to/splits \
      --base_output_dir /path/to/outputs/{target_gene} \
...
def custom_collate_fn(batch):
    ...
    for item in batch:
        ...
        if isinstance(label, dict):
            ...
            if 'label' in label:
                # For brca_lusc dataset (binary classification)
...
def train_single_gpu(split_idx, feats_path, splits_dir, output_base_dir, device,
                     input_feature_dim, num_epochs, model_name, target_gene, mutation_labels_file, batch_size=8):
    """
    ...
    Args:
        ...
        target_gene: Target gene for binary classification
        mutation_labels_file: Path to mutation labels CSV file
    ...
    if not BRCA_LUSC_AVAILABLE:
        print("Error: brca_lusc_dataset not available")
        return None
    
    print(f"Features path: {feats_path}")
    print(f"Target gene: {target_gene}")
    print(f"Mutation labels file: {mutation_labels_file}")
    ...
    # Model - binary classification for gene mutation
    n_classes = 2
...
# -------------------------------
def main():
    parser = argparse.ArgumentParser(description='ABMIL Training Script for Gene Mutation Classification (Single GPU)')
    ...
    parser.add_argument('--base_output_dir', type=str, required=True,
                        help='Base output directory for saving results. Supports {target_gene} placeholder.')
    ...
    for MODEL_NAME, model_params in MODEL_HYPERPARAMS.items():
        ...
        # Support {target_gene} placeholder in base_output_dir
        if '{target_gene}' in args.base_output_dir:
            base_output_dir = args.base_output_dir.format(target_gene=args.target_gene)
        else:
            base_output_dir = args.base_output_dir

        print(f"\n{'='*80}")
        print(f"Training model: {MODEL_NAME} for gene mutation classification")
        ...
        if not os.path.exists(feats_path):
            print(f"[skip] Features path does not exist: {feats_path}")
            continue

        if not os.path.exists(splits_dir):
            print(f"[skip] Splits directory does not exist: {splits_dir}")
            continue

        if not os.path.exists(args.mutation_labels_file):
            print(f"[skip] Mutation labels file does not exist: {args.mutation_labels_file}")
            continue

        os.makedirs(base_output_dir, exist_ok=True)

        # Run training for each split
        num_splits = 5
        all_results = []
        ...
        # Save summary across all splits
        if all_results:
            summary = {
                'target_gene': args.target_gene,
                'model_name': MODEL_NAME,
                'num_splits': len(all_results),
                'mean_test_auc': float(np.mean([r['test_auc'] for r in all_results])),
                'std_test_auc': float(np.std([r['test_auc'] for r in all_results])),
                'mean_test_acc': float(np.mean([r['test_acc'] for r in all_results])),
                'std_test_acc': float(np.std([r['test_acc'] for r in all_results])),
                'mean_test_f1': float(np.mean([r['test_f1'] for r in all_results])),
                'std_test_f1': float(np.std([r['test_f1'] for r in all_results])),
                'mean_best_val_auc': float(np.mean([r['best_val_auc'] for r in all_results])),
                'std_best_val_auc': float(np.std([r['best_val_auc'] for r in all_results])),
                'mean_best_val_acc': float(np.mean([r['best_val_acc'] for r in all_results])),
                'std_best_val_acc': float(np.std([r['best_val_acc'] for r in all_results])),
                'mean_best_val_f1': float(np.mean([r['best_val_f1'] for r in all_results])),
                'std_best_val_f1': float(np.std([r['best_val_f1'] for r in all_results])),
            ...
            print(f"Mean Test AUC: {summary['mean_test_auc']:.4f} ± {summary['std_test_auc']:.4f}")
            print(f"Mean Test Acc: {summary['mean_test_acc']:.4f} ± {summary['std_test_acc']:.4f}")
            print(f"Mean Test F1: {summary['mean_test_f1']:.4f} ± {summary['std_test_f1']:.4f}")
            print(f"{'='*80}")
